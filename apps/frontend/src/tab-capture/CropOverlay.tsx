...
export const CropOverlay = forwardRef<CropOverlayHandle, CropOverlayProps>(
  function CropOverlay({ drawing, crop, onCropChange, children }, ref) {
    ...
    useImperativeHandle(ref, () => ({
      setCapturing(value: boolean) {
        ...
        if (!el) return;
        if (value) el.setAttribute("data-capturing", "true");
        else el.removeAttribute("data-capturing");
      },
    }));
    ...
    return (
      <Box
        ...
        sx={{
          ...
          width: "100%",
          height: "100%",
          '&[data-capturing="true"] [data-testid="tab-capture-crop-rect"]': {
            visibility: "hidden",
            opacity: 0,
